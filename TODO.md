...
- [ ] Секция «разблокировано со вчера» в дайджесте: пункты TODO.md, чей `@blocked_by`-таргет закрылся с прошлого дайджеста. По прецеденту #42 читать машинный вердикт флотового plan-check (PF-BLOCKER-STALE), не пересчитывать PF-семантику по зеркалам; недоступный вердикт — явный unknown, не молчаливый пропуск (unknown ≠ «ничего не разблокировано»); нет разблокированных — секция явно пустая или отсутствует. Запрос prograph-vault (правило `cross-repo-waits`, PR prograph-vault#72), issue #47 @owner:github:andrei-shtanakov @id:digest-unblocked-section
